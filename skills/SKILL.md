...
| What did X say in a Teams chat | `find-chats-with-user --name '<person>'` → `list-teams-chat-messages --chat-id '<id>' --skip-system true` (add `--mentions-me true` for what was asked of the user; every message carries `webUrl`); or `list-teams-chats-with-messages` for recent chats with bodies inlined. Chat content is not in federated search, so this is the only route |
