...
const createTokenCallbackServer = (logger: Logger, timeoutMs: number = 5 * 60 * 1000): TokenCallbackServer => {
  ...
  // Resolve the pending start() promise at most once, then tear down.
  const settle = (result: Result<TokenCallbackPayload, TokenCallbackError>): void => {
    if (resolveCallback) {
  ...
  // The `/token` POST body is streamed; `end` parses + validates it. Hoisted
  // to this scope (not nested in the createServer handler) to keep callback
  // nesting shallow.
  const handleTokenPost = (req: IncomingMessage, res: ServerResponse): void => {
    let body = '';
    req.on('data', (chunk) => {
      body += chunk;
      if (body.length > 1e6) {
  ...
  // Bind to port 0 — OS assigns a random available port. Hoisted out of the
  // start() executor so its setTimeout callback stays shallow.
  const onListening = (resolve: (result: Result<TokenCallbackPayload, TokenCallbackError>) => void): void => {
    ...
    timeoutHandle = setTimeout(() => {
      ...
      settle(err({ type: 'timeout', message: `token callback timed out after ${timeoutMs}ms` }));
    }, timeoutMs);
  };
  ...
  const start = (): Promise<Result<TokenCallbackPayload, TokenCallbackError>> => {
    return new Promise((resolve) => {
      ...
      server.listen(0, '127.0.0.1', () => onListening(resolve));
      server.on('error', (e) => {
        const message = e instanceof Error ? e.message : String(e);
