 ...
 * heavier dependency). Every page is walked (PDF has no "hidden page" concept) unless
 * `includePage` narrows the walk, which also skips decoding the pages left out.
 *
 ...
 */
const extractPdfImages = async (bytes: Uint8Array, includePage: (page: number) => boolean = () => true): Promise<Result<ReadonlyArray<MediaPart>, GraphError>> => {
  try {
    const { extractImages, getDocumentProxy } = await import('unpdf');
    const doc = await getDocumentProxy(bytes);
    const parts: Array<MediaPart> = [];
    for (let page = 1; page <= doc.numPages; page += 1) {
      if (!includePage(page)) continue;
