...
// A /users read resolves on the BASIC token: `User.ReadBasic.All` plus the tenant's default
// member-read reaches jobTitle / department / office / phones / manager (probed live 2026-08-14).
// This is the common path and needs no browser login. Fall back to the elevated M365 token ONLY
// on a 403 — a tenant that restricts basic directory reads. The trigger is the 403, never a
...
// id / UPN / email → the FULL directory profile (basic token, elevated fallback via getProfile).
...
const resolveByIdentifier = async (graph: GraphClient, userId: string, query: Parameters<typeof appendOData>[1]): ReturnType<Command['execute']> => {
  ...
  const byMail = await getProfile(graph, appendOData(`/users?$filter=mail eq '${odataStringLiteral(userId)}'`, query));
  ...
  options: [
    {
      ...
      required: true,
      description:
        'Azure AD user ID, UPN, or email (returns the full profile via the basic token, falling back to the elevated token only on tenants that restrict basic directory reads), OR a display name (returns relevant-people candidates on the basic token). Discover ids via `list-relevant-people` or `microsoft-search-query`.',
    },
    ...selectExpandOptions,
  ],
