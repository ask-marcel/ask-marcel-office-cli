...
| Friction-plan #5 — one shared `--drive-id` description constant (supersedes QA-003) | P3 | friction-plan 2026-06-16 | **DEFERRED 2026-06-16**: inspection found 31 inline drive-id defs, no shared helper → a 24-file DRY refactor + manifest regen for a minor gain (11 terse-variant cmds lack the `list-drives` pointer; variants 3/4/5 are intentionally tailored). Low value / high churn — left for a focused pass |
