...
| `convert-mail-attachment-to-pdf` / `convert-calendar-event-attachment-to-pdf` (temp-upload → `?format=pdf`) | distinct path from drive→pdf: uploads the attachment to a temp `/me/drive` folder, runs `?format=pdf`, then deletes — its own failure modes, not covered by drive→pdf | **OPEN P2 — 2026-07-05 (2.0.0): `convert-mail-attachment-to-pdf` 406s on XLSX attachments.** Isolated: pptx→pdf ✓, pdf→pdf ✓, xlsx→markdown ✓, and **drive xlsx→pdf (`download-drive-item-as-pdf`) ✓** — but xlsx *mail attachment*→pdf returns `HTTP 406 /transform/pdf`. So it's the temp-upload path, xlsx-specific (likely the temp `/me/drive` item's name drops the `.xlsx` extension, so `?format=pdf` can't infer the input format). Fix candidate: preserve the source extension on the temp upload. |
