...
| Downstream plugin doc sweep — stale `>/dev/null`/GBK-unzip workarounds + phantom flags (`--link`,`--start`,`get-event`,`--filter`,`--user-email`) across 13 files | — | QA 2026-06-15 | partially done (plugin e53d622 fixed the 5 phantom flags); residual `>/dev/null` ×8, GBK ×4, **+3 NEW stale `download-onedrive-file-content` refs** from the 1.5.0 rename — carry |
...
| `read-mail-attachment` routes by filename extension despite a "by content-type" summary (real `.jpg`-named xlsx → image-415) | P3→F | QA E-02 (2026-06-16) | summary reworded to "by file type" (FIXED). **F option (open):** prefer contentType when ext says image but contentType is a known Office type, so misnamed attachments actually convert |
